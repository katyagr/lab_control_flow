...
public class ControlFlowExercises {
    ...
    boolean kidsRide(int temp, boolean isWinter) {
        ...
        if (10 <= temp && isWinter && temp <= 30){
            return true;
        } else if(15 <= temp && !isWinter && temp <= 30){
            return true;
        } else{
            return false;
        }
    ...
    boolean find8(int a, int b) {
        ...
        if(a == 8 || b == 8 || a + b == 8 || a - b == 8 || b - a == 8 || a*b == 8){
            return true;
        } else{
            return false;
        }
    ...
    boolean hangPaintings(int small, int big, int length) {
        ...
        if(length - 5*big <= small && length - 5*big >= 0){
            return true;
        ...
        /* this solution doesn't account for all possible situations just all the ones listed here -
        it does not account for situations where you would use some but not all of the big paintings */
    }
    ...
    String canParty(int day, boolean vacation) {
        ...
        if(!vacation && (day <= 4 || day == 7)){
            return "Don't party";
        }else if((!vacation && (day == 5 || day == 6)) || vacation && (day <= 4 || day == 7)){
